...
const MovieReviews = ({movieID}) => {
    ...
    async function getReviews() {
        const rawData = await axios.get(`https://api.themoviedb.org/3/movie/${movieID}/reviews?api_key=cfd7e9d93a8159c720cab16e6382e3eb`);
    ...
    return (
        <div className="details-reviews-container">
            ...
            {(reviews!==undefined) ?
                (reviews.length!==0 ? reviews.map((item) => (<MovieReview key={item.id} review={item} />)) :
